...
def cancel_mirror(update, context):
    ...
    if len(args) > 1:
        ...
        if not dl:
            sendMessage(f"GID: <code>{gid}</code> <b>Not Found.</b> ❗️", context.bot, update)
            return
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"<b>Please reply to the</b> <code>/{BotCommands.MirrorCommand}</code> <b>message which was used to start the download or send</b> <code>/{BotCommands.CancelMirror} GID</code> <b>to cancel it!</b>"
        if mirror_message and mirror_message.message_id not in keys:
            if BotCommands.MirrorCommand in mirror_message.text or \
               ...
               BotCommands.UnzipMirrorCommand in mirror_message.text:
                msg1 = "<b>Mirror Already Have Been Cancelled</b> ❗️"
    ...
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("<b>Archival in Progress, You Can't Cancel It.</b> 🤷‍♂", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("<b>Extract in Progress, You Can't Cancel It.</b> 🤷‍♂", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_SPLITTING:
        sendMessage("<b>Split in Progress, You Can't Cancel It.</b> 🤷‍♂", context.bot, update)
    else:
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'<code>{count}</code> <b>Download(s) has been Cancelled!</b> ❌', context.bot, update)
